import sequtils, unicode, vmath, bumpy, pixie
...
proc locationRect*(textBox: TextBox, loc: int): Rect =
  ...
  let layout = textBox.glyphs.selectionRects
  if layout.len > 0:
    if loc >= layout.len:
      let g = layout[^1]
      let r = textBox.glyphs.runes[^1]
      # if last char is a new line go to next line.
      if $r == "\n":
        result.x = 0
        result.y = g.y + textBox.font.lineHeight
      else:
        result = g
        result.x += g.w
    else:
...
proc scrollBy*(textBox: TextBox, amount: float) =
  ...
  if amount > 0:
    ...
    textBox.scroll.y = min(0,textBox.scroll.y)
  else:
    # Or the bottom.
    textBox.scroll.y = max(float(textBox.height - textBox.innerHeight), textBox.scroll.y)
